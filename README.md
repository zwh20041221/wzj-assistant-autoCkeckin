...
- 定位签到：按配置 `lat/lon` 提交坐标签到；未配置时尝试无坐标；支持自定义延迟 `start_delay_gps`
- 普通签到：直接提交，不带经纬度；支持自定义延迟 `start_delay`
- 日志：时间戳前缀；`debug=1` 打印 RAW 帧、心跳 ACK、逐条消息等细节；签到响应完整打印
...
- `start_delay`：普通签到（非 GPS/QR）检测到后的延迟时间（毫秒）
- `start_delay_gps`：定位签到检测到后的延迟时间（毫秒）
- `start_delay_qr`：二维码签到检测到后的延迟时间（毫秒）
