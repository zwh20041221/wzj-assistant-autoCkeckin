  ...
  return await activeSign(ctx.openId)
    .then(async (data) => {
      ...
      for (const sign of queue) {
        ...
        console.log(
            `[TS][Detect] ${name} (signId=${signId}, courseId=${courseId}) @ ${new Date().toISOString()}`
          );
        ...
        // timestamp: detect QR sign-in activity
        ...
        if (isQR) {
          ...
          ctx.qrSign?.destory();
          // 优先复用预握手长连，避免首次 connect 延迟
          if (ctx.qrWarm) {
            ctx.qrWarm.attach({ courseId, signId });
            ctx.qrSign = ctx.qrWarm;
          } else {
